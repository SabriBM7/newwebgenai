...
import type { MenuItem, HeaderStyle, Animation } from "@/types/index"
        ...
        <header className={headerClasses} style={headerStyles} data-keywords={keywords.join(",")}>
            <div className="container mx-auto px-4 h-full flex items-center justify-between">
                <div className="flex items-center">
                    <SafeLink href="/" className="font-bold text-xl flex items-center">
                        {logoUrl ? (
                            <Image src={logoUrl || "/placeholder.svg"} alt={logo || "Logo"} width={40} height={40} className="mr-2" />
                        ) : (
                            <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center text-white mr-2">
                                {logo && typeof logo === "string" && logo.length > 0 ? logo.charAt(0) : "L"}
                            </div>
                        )}
                        <span>{logo || "Logo"}</span>
                    </SafeLink>
